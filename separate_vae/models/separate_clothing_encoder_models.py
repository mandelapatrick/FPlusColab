...
class VAE_MODEL(torch.nn.Module):
    ...
    def initialize(self, opt):
        ...
        self.device = torch.device('cpu') # torch.device('cuda:{}'.format(self.gpu_ids[0])) if self.gpu_ids else torch.device('cpu')
    ...
    def one_hot_tensor(self, label_map):
        ...
        input_label = input_label.scatter_(1, label_map.data.long(), 1.0) #.cuda(), 1.0)
        return input_label
